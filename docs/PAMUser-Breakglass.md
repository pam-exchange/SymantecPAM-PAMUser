...
> [!NOTE]
> In the example here a new PAM user is created. A similar setup can be used for the built-in administrator `super`. 
...
An important update is the password length for PAM users. Default is 16 characters, which is insufficient for the passwords generated by PAM. Update the Global Settings and change the password length to 72 characters (maximum length).
